<template>
  <div class="main-page-container">
    ...
    <div class="bottom-half-container">
      <form @submit.prevent="handleRegisterSubmit" class="auth-content-wrapper">
        ...
        <button type="submit" class="main-register-button">REGISTER</button>


        <div class="already-account mt-3 text-sm">
          ALREADY HAVE AN ACCOUNT? <RouterLink to="/login" class="link-login">LOGIN</RouterLink>
        </div>
      </form>
    </div>
  </div>
...
// 3. Правила валідації (більш суворі для реєстрації)
const validationRules: Record<FormFields, (value: string) => string> = {
  username: (value: string) => {
    if (!value) return 'Ім\'я користувача обов\'язкове.';
    if (value.length < 4) return 'Ім\'я повинно мати принаймні 4 символи.';
    return '';
  },
  password: (value: string) => {
    if (!value) return 'Пароль обов\'язковий.';
    if (value.length < 6) return 'Пароль повинен мати принаймні 6 символів.';
    return '';
  },
  email: (value: string) => {
...
const handleRegisterSubmit = () => {
  if (validateForm()) {
    console.log('✅ Реєстрація успішна! Дані форми:');
    ...
    console.log('❌ Реєстрація не вдалася. Будь ласка, виправте помилки.');
  }
};
</script>
...
.tab-login { background-color: var(--main-red); color: white; margin-right: 5px; }
.tab-register { background-color: white; color: var(--main-red); margin-left: 5px; }
...
.already-account { margin-top: 10px; }
